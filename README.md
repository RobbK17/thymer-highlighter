...
**Version:** `1.0.7` (in `plugin.json` as `version`). The `ver` field is the manifest / API version expected by Thymer. The same number appears as `PLUGIN_VERSION` at the top of `plugin.js` — keep them aligned when releasing.
...
- **1.0.5** — Removed workspace export dry-run; use **Rebuild local index** for read-only refresh. Refactors: `HIGHLIGHT_HEADING_LEVELS` / `buildHeadingHighlightRules`, `highlighterToaster`, `_paletteCommands` (eight commands). `_lookupRecordTitleByGuidFromPrompt` remains in source only.
